...
\begin{figure}[ht]
    ...
    \caption{Accuracy and Precision. Source: \cite{misc:acc-prec-tobii-2023}}
    \label{fig:acc-prec-tobii-2023}
\end{figure}
...
\begin{itemize}
    ...
    In this field, eye tracking allows for the analysis of the behavior of the user. This is a process that occurs after they have used the product, which means it does not take an active role in the interaction per se. \citep{art:commentary-eyetrack-2023}

    \item Psychology experiments
    
    Many different psychology experiments can be done using eye tracking, for instance, the cognitive load students using a Learning Management System (LMS) has been analyzed using eye tracking, and how it had an effect on their learning outcomes. \citep{art:eyetrack-students-2024}

    \item Input method
    
    As \cite{art:commentary-eyetrack-2023} point out, the application of eye trackers in HCI (Human-Computer Interaction) has long been studied and researched. The approach of using eye tracking as an input method is yet to be applied massively in a commercial environment.

    \item Artificial Intelligence
    
    In the rising field of Generative Artificial Intelligence, \cite{art:mining-eyetracking-2024} used eye tracking to perform text summarization, mining data from attention metrics, which outperforms regular text summarization. 
...
%     NLP powers search engines by enabling them to interpret user queries, rank relevant content, and provide accurate results. Techniques like query expansion, semantic search, and text ranking are key components %\citep{inproc:search-nlp-2021}.

%     \item Assistive Technologies
    
%     Assistive tools such as screen readers, language translation systems, and real-time transcription services leverage NLP to improve accessibility for individuals with disabilities 
%     % \citep{art:assistive-nlp-2023}.

%     \item Transcriptions

%     Automated transcription services use NLP to convert spoken language into text. These systems rely on speech recognition combined with language modeling to ensure accurate transcription 
%     % \citep{misc:speech-transcription-2022}.

%     \item Smart Voice Assistants

%     Smart assistants like Siri, Alexa, and Google Assistant are powered by NLP, enabling them to understand voice commands, process natural language queries, and generate spoken responses 
%     % \citep{art:voice-nlp-2024}.
% \end{itemize}
